	...
	while(flag)
	{
		...
		for(int j = 0; j < THREAD_NUM; j++)
		{
			while(findreduce(data[j].seq))
			{
				...
				//cout<< "-------------------------reduced with register: seq.size() = " << data[j].seq.size() - SIZE <<endl;
			}
...
void reduce_gap(vector<list> &seq)
{
	...
	{   
		...
		{  
			...
			{
				update_seq(seq);
				//cout<< "-------------------------reduced with register: seq.size() = " << seq.size() - SIZE <<endl;
			}
...
int get_reduced(vector<list> &o_seq)
{
	...
	{  
		...
		//cout<< "---------------After step_1 (with registers): seq.size() = " << seq.size() - SIZE <<endl;
	    if((check(seq)) && ((seq.size() - SIZE) < MAX_SIZE))
	    //if((seq.size() - SIZE) < MAX_SIZE)
	    {	
	    	MAX_SIZE = seq.size() - SIZE;
...
void get_equivalent_circuit(vector<list> &seq, int sp, int length)
{
	...
	/*cout<<"print segment"<<endl;
	for(int i = 0; i < tmp_seq.size(); i++)
	{
		cout<<"flag_"<<i<<" = "<<tmp_seq[i].flag<<" ： ";
		for(int j = 0; j < tmp_seq[i].value.size(); j++)
		{
			cout<<tmp_seq[i].value[j]<<"  ";
		}
		cout<<endl;
	}*/
	...
	int sp_B = input.size();
	/*cout<<"after B algorithm, e_seq is:"<<endl;
	for(int i =0; i < e_seq.size(); i++)
	{cout<<"flag_"<<i<<" : = "<<e_seq[i].flag<<":  ";
		for(int j = 0; j< e_seq[i].value.size(); j++)
		{
			cout<<e_seq[i].value[j]<<" ";
		}
		cout<<endl;
	}*/
	//seq.insert(seq.begin() + sp, e_seq.begin() + NumTargets, e_seq.end());
	seq.insert(seq.begin() + sp, e_seq.begin() + sp_B, e_seq.end());
/*	
cout<<"sp = "<<sp<<"sp_B = "<<sp_B<<"dst = "<<dst<<endl;
cout<<"after B algorithm, seq is:"<<endl;
	for(int i =0; i < seq.size(); i++)
	{cout<<"flag_"<<i<<" : = "<<seq[i].flag<<":  ";
